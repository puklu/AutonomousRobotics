...
class PathFollower(object):
    def __init__(self):
        ...
        self.max_velocity = rospy.get_param('~max_velocity', 0.08)  # maximumm allowed velocity (m/s)
        self.max_angular_rate = rospy.get_param('~max_angular_rate', 0.3)  # maximum allowed angular rate (rad/s)
        self.look_ahead_dist = rospy.get_param('~look_ahead_dist', 1.50)  # look ahead distance for pure pursuit (m)
        ...
        self.front_limit = 0.3
        self.previoud_heading_error = 0.0  # heading error in a previous step
        self.heading_error_integrated = 0.0  # integrated heading error
    ...
    def getLookaheadPoint(self, pose):
        ...
        print("...distance and lookahead..")
        rospy.loginfo(distance)
        rospy.loginfo(self.look_ahead_dist)
        rospy.loginfo(self.path_index)

        # the goal is within the lookahead distance
        if distance < self.look_ahead_dist and self.path_index + 1 == len(self.path):
            print("....hello from distance < lookahead and last element....")
            goal = next_wp
        ...
        # elif distance < self.look_ahead_dist:
            ...
            while this_distance < self.look_ahead_dist:
                ...
                this_distance = np.linalg.norm(point_to_consider - pose2)
    ...
    # to compute ranges from lidar data
    def compute_front_range(self, msg, angle_min, angle_max):
        ranges = np.array(msg.ranges)
        range_1 = ranges[(360 + angle_min):360]
        range_2 = ranges[0:angle_max]
        ranges = np.hstack((range_1, range_2))
        ranges = ranges[np.isfinite(ranges)]
        ranges = ranges[msg.range_min < ranges]
        ranges = ranges[msg.range_max > ranges]

        return ranges.min()

    def compute_range(self, msg, angle_min, angle_max):
        ranges = np.array(msg.ranges)
        ranges = ranges[(360 + angle_min) % 360:(360 + angle_max) % 360]
        ranges = ranges[np.isfinite(ranges)]
        ranges = ranges[msg.range_min < ranges]
        ranges = ranges[msg.range_max > ranges]
    ...
    def scan_cb(self, msg):
        """Scan callback updates distance to obstacles around"""
        # front angles
        angle_min = -30
        angle_max = 30
        # front minimum range
        self.front_range = self.compute_front_range(msg, angle_min, angle_max)
        # left view angles
        angle_min = 30
        angle_max = 90
        self.left_range = self.compute_range(msg, angle_min, angle_max)
        # right vew angles
        angle_min = -90
        angle_max = -30
        self.right_range = self.compute_range(msg, angle_min, angle_max)
    ...
    def control(self, msg):
        ...
        # TODO: implement correct reaction if empty path is received
        if self.path_msg is None:
            # TODO student's code
            #################
        ...
        while True:
            ...
            if (np.linalg.norm(pose[0:2] - goal[0:2]) >= self.max_path_dist) and not self.started:
            ...
            # Position displacement(direction and Euclidean distance)
                ...
                if goal_dist == 1e-2:  # FIXME: replace by your code
                    velocity = 0
                    angular_rate = 0
                    # TODO student's code
