...
class Evaluathor():
    ...
    def compareMaps(self):
        ...
        # shift = (np.r_[self.mapInfo.origin.position.x, self.mapInfo.origin.position.y] - self.gtShift) / self.mapInfo.resolution + np.r_[1, 1]
        ...
        mapPad = self.__padImage2Shape(transformedMapImage, finalShape, -1)
        gtPad = self.__padImage2Shape(self.gtImage, finalShape, -1)
        ...
        missed = np.where(np.logical_and(mapConv == -1, gtConv == 1))
        diffGrid[missed] = self.D_MISSED
        # other -> (map == occup & (gt == empty | gt == unknown)) | (map == unknown & gt == empty) 
        other = np.where(np.logical_or(
            np.logical_and(mapConv == 1, np.logical_or(
                gtConv == 0,
                gtConv == -1)),
            np.logical_and(mapConv == -1, gtConv == 0)))
        diffGrid[other] = self.D_OTHER
        ...
        # compute score
       # occ_plus = np.where(np.logical_and(mapConv == 1, gtConv == 1))
        #occ_minus = np.where(np.logical_or(np.logical_and(mapConv == 0, np.logical_or(gtConv == 1, gtConv == -1)), np.logical_and(mapConv == 1, np.logical_or(gtConv == 0, gtConv == -1))))

        plus_score = np.where(np.logical_or(np.logical_and(mapConv == 0, gtConv == 0),np.logical_and(mapConv == 1, gtConv == 1)))
        mappingScore = np.count_nonzero(plus_score)-np.count_nonzero(minus)-np.count_nonzero(other)-np.count_nonzero(missed)

        actualScore = mappingScore

        plus_gt = np.where(np.logical_or(gtConv == 0, gtConv == 1))
        gtScore = np.count_nonzero(plus_gt)
        score_percent = int(np.round(100.0*mappingScore/gtScore))
        ...
        gray = 196
        scoreScreen = self.__putText(scoreScreen, "mapping score= {:5d}%".format(score_percent), (xp * 3, 10), color=(gray, gray, gray), size=0.6, thickness=1)
        # add time
        ...
        if (currentTime > 0 and ((currentTime % self.nextEvalTime) == 0)) or currentTime >= self.timeLimit:
            ...
            self.data[-1][self.intervalFields[np.round(currentTime / self.timeInterval).astype(np.int32) - 1]] = score_percent
            rospy.loginfo("Scoring interval <{}..{}>s had passed, writing current score:\n"
                          "\ttotal = {}\n\t\tmapping = {}".format(self.nextEvalTime - self.timeInterval, self.nextEvalTime,
                                                                                   actualScore, mappingScore))
        ...
        if currentTime >= self.timeLimit:
            ...
            pointsPercent = [self.data[-1][ifield] for ifield in self.intervalFields]
            averageMappingPoints = np.round(np.average(pointsMapping))
            averagePercent = np.round(np.average(pointsPercent))
            self.data[-2]["per run"] = averageMappingPoints
            self.data[-1]["per run"] = averagePercent
            pointsSum = averageMappingPoints
            percentSum = averagePercent
        ...
        if key == ord("r"):  # restart simulation
            # if self.runMode == "auto":
            #     self.mapIndex -= 1
            # rospy.logdebug("Restarting simulation.")
            self.restart()
            rospy.logdebug("Restarting is temporalily removed.")
    ...
    def restart(self):
        ...
        self.data.append({"score": "percent"})
