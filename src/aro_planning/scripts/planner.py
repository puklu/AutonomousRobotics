...
class PathPlanner():
    ...
    def getGridPosition(self, pose2d):
        pos = np.array([pose2d.x - self.grid_info.origin.position.x, pose2d.y - self.grid_info.origin.position.y, 0, 1])
    ...
    def planPath(self, request):
        ...
        print("hello from planPath")

        startPosition = self.getGridPosition(request.start)
        goalPosition = self.getGridPosition(request.goal)
        print("start")
        rospy.loginfo(startPosition)
        print("goal")
        rospy.loginfo(goalPosition)
        ...
        # computing the path using A-star
        path = []
        path_history = {}
        path_cost = {}
...
###############################################################################
        ...
        if len(path) == 0:
            ...
            real_path = [Pose2D(pos[0], pos[1], 0) for pos in
                         [self.gridToMapCoordinates(np.array(waypoint) + np.array([0.5, 0.5])) for waypoint
                          in
                          path]]
    ...
    def grid_cb(self, msg):
        ...
        if not self.gridReady:
            # Create services
            self.path_planner_service = rospy.Service('plan_path', PlanPath, self.planPath)
            self.gridReady = True
