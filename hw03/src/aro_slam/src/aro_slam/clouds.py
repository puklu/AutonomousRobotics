...
def position_and_normal(x_struct):
    pos = structured_to_unstructured(x_struct[Channels.position.value])
    norm = structured_to_unstructured(x_struct[Channels.normal.value])
    return np.hstack((pos, norm))
